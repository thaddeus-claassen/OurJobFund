...
class JobUserTestCase(TestCase):
    ...
    def setUp(self):
        ...
        user.save();
        job = Job.create(name="job_in_jobuser");
        job.save();
        jobuser = JobUser.create(user=user, job=job, pledged=1234, paid=1234, work_status="Working", received=1234);
        jobuser.save();
        
    def test_jobuser(self):
        user = get_object_or_None(User, username="user_in_jobuser");
        job = get_object_or_None(Job, name="job_in_jobuser");
