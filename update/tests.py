...
class UpdateTestCase(TestCase):
    ...
    def setUp(self):
        ...
        user.save();
        job = Job.create(name="job_in_update");
        job.save();
        jobuser = JobUser.create(user=user, job=job);
        jobuser.save();
        update = Update.create(jobuser, description="zxcvbnmasdfghjklqwertyuiopZXCVBNMASDFGHJKLQWERTYUIOP,./;'[]\"\\<>?L:{}|1234567890`-=~!@#$^&*()_+");
        update.save();
        
    def test_jobuser(self):
        user = get_object_or_None(User, username="user_in_update");
        job = get_object_or_None(Job, name="job_in_update");
